...
class SpinnerAdapter(val context: Context, var dataSource: Array<String>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (convertView == null) {
            view = if(position==0)
                ...
                inflater.inflate(R.layout.spinner_item_dropdown, parent, false)
            vh = ItemHolder(view, position)
            view?.tag = vh
    ...
    private class ItemHolder(row: View?, position: Int) {
        val label: TextView = if(position==0)
            ...
            row?.findViewById(R.id.txt_spinner_dropdown) as TextView
